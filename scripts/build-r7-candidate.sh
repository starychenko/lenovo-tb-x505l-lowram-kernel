...
usage() {
    echo "Usage: $0 <r7-kernel-source> <clang-r365631c-dir> <aarch64-gcc-4.9-dir> <config> <output-dir> [reproducibility-key.pem]" >&2
}
...
build_timestamp="${TB_X505L_BUILD_TIMESTAMP:-Mon Aug 31 10:45:00 UTC 2026}"
build_version="${TB_X505L_BUILD_VERSION:-14}"
build_jobs="${TB_X505L_BUILD_JOBS:-8}"
build_user="${TB_X505L_BUILD_USER:-codex-r7}"
...
if [[ "${kernel_version}" != "4.9.337" ]]; then
    echo "Refusing a non-r7 source version: ${kernel_version}" >&2
    exit 1
fi
...
if ! grep -q '^CONFIG_LOCALVERSION="-tbx505l-r7-4.9.337-compat-vendor"$' \
    "${config_source}"; then
    echo "Refusing a config without the exact qualified r7 local version." >&2
    exit 1
fi
